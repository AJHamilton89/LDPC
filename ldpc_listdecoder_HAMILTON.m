...
%%set LLRs of least confident bit to flip and be a very high value
     
    if isempty(find((sum(H,2)==(min(abs(sum(H,2))))))) == 1
    least_conf_location=find((sum(H,2)==-(min(abs(sum(H,2))))));
    else
    least_conf_location=find((sum(H,2)==(min(abs(sum(H,2))))));
    end
%     least_conf_location=   find(sums==(sign(min(sums))*(min(abs(sums)))));
    
    value=-(sign(sum(H(least_conf_location,:))))*100000;
    H(least_conf_location,:)=value;
%     H(least_conf_location)=value;
     for c = 1:num_c
        for v = 1:num_v
            if matrix(c,v)==0
